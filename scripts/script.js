...
function Grades() {
  ...
  const template = document.querySelector("#navigation__grades-template")
    .content;
...
function updateSearchResultsCount() {
  const resultsCount = document.querySelectorAll(".main__list__item_is-opened")
    .length;
...
function showFilmCard(item) {
  ...
  if (item.season) {
    ...
    if (item.continuation) {
      headerElements.continuationYear.textContent = item.continuation.getFullYear();
  ...
  const filmTemplate = document.querySelector("#film__screenshots-template")
    .content;
